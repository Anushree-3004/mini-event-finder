...
export default function MapPicker({
  ...
  const [initialCenter, setInitialCenter] = useState<Coords | null>(
    center ?? null
  );
  useEffect(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        setInitialCenter({ lat: 22.91202, lng: 72.74414 }); // fallback: SF
      },
  ...
  const mapCenter = useMemo<LatLngExpression>(() => {
    const c = current || initialCenter || { lat: 22.91202, lng: 72.74414 };
    return [c.lat, c.lng];
